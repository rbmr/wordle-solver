...
- The number of possible responses $|R| = 3^\texttt{N_CHARS}$
- To distinguish among $|C|$ possibilities requires at least $\log_{|R|}|C|$ responses in expectation. $E(C) \geq \log_{|R|}|C|$ 
- $\log_{|R|}|C| = \log_2|C| / \log_2|R| = \gamma \cdot \log_2|C|$ where we precompute $\gamma = 1 / \log_2|R| = 1 / \log_2 3^\texttt{N_CHARS} = \texttt{N_CHARS} / \log_2 3$
